...
import static com.aliyun.jindodata.plugin.oss.maven.matchers.Matchers.eq;
...
public class OssWagonTest {
    ...
    private final OssWagon wagon = new OssWagon(this.amazonS3, BUCKET_NAME, BASE_DIRECTORY);

    // Run only locally against own aws keys
    @Ignore
    @Test
    public void regionConnections() throws WagonException {
        OssWagon remoteConnectingWagon = new OssWagon();
    ...
    public void doesRemoteResourceExistExists() {
        when(this.amazonS3.getObjectMetadata(OssWagonTest.BUCKET_NAME, BASE_DIRECTORY + FILE_NAME))
    ...
    public void getResource() throws TransferFailedException, FileNotFoundException, ResourceDoesNotExistException {
        when(this.amazonS3.getObject(OssWagonTest.BUCKET_NAME, BASE_DIRECTORY + FILE_NAME))
